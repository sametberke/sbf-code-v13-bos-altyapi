//karışanın taaaa karışma buraya meraklı komutlar klasörüne git hadi işine
const commands = [];
        ...
        if(command.option) {
          for(i = 0; i < command.option.length; i++) {
            if(!command.option[i].choices) {
              ...
              if(command.option[i].type === 'BOOOLEAM') slashCommand.addBooleanOption(option => option.setName(command.option[i].name).setDescription(command.option[i].description).setRequired(command.option[i].require))
